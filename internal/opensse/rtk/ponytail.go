...
var PonytailPrompts = map[string]string{
	...
	PonytailFull: ponytailPersona + " Full: the ladder enforced. Stdlib and native first. Shortest diff, shortest explanation. " +
		ponytailLadder + " " + ponytailRules + " " + ponytailOutput + " " + ponytailNotLazy + " " + ponytailPersist,
	PonytailUltra: ponytailPersona + " Ultra: YAGNI extremist. Deletion before addition. Ship the one-liner and challenge the rest of the requirement in the same response. " +
		ponytailLadder + " " + ponytailRules + " " + ponytailOutput + " " + ponytailNotLazy + " " + ponytailPersist,
}

// InjectPonytail injects ponytail system prompt.
func InjectPonytail(body map[string]any, format, level string) {
	if level == "" {
		level = PonytailFull
	}
