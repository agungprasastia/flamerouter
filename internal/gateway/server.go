// Package gateway implements the HTTP API routing, middleware, and management endpoints for flamerouter.
package gateway
...
// Server is the HTTP server for flamerouter gateway.
type Server struct {
	cfg      *config.Config
	st       *store.Store
	keys     *auth.APIKeys
...
func (s *Server) routesProviders() {
	...
	s.mux.HandleFunc("GET /api/providers/registry", s.handleProviderRegistry)
	...
	s.mux.HandleFunc("PUT /api/providers/{id}", s.handleUpdateProvider)
...
func (s *Server) routesModelsAndPools() {
	...
	s.mux.HandleFunc("DELETE /api/models/custom", s.handleDeleteCustomModel)
...
func (s *Server) routesCLIToolsAndOps() {
...
func (s *Server) routesTunnelsAndUsage() {
...
func (s *Server) routes() {
	...
	s.routesSettings()
	s.routesProviders()
	s.routesModelsAndPools()
	s.routesCLIToolsAndOps()
	s.routesTunnelsAndUsage()
	s.routesV1API()
}
...
func (s *Server) handleHealth(w http.ResponseWriter, _ *http.Request) {
	...
	_, _ = w.Write([]byte(`{"ok":true}`)) //nolint:errcheck // best effort write
}
...
func (s *Server) handleKeys(w http.ResponseWriter, r *http.Request) {
	...
	case http.MethodGet:
		...
		w.Header().Set("Content-Type", "application/json")
		_ = json.NewEncoder(w).Encode(map[string]any{"keys": list}) //nolint:errcheck // best effort encode
	case http.MethodPost:
		...
		w.Header().Set("Content-Type", "application/json")
		_, _ = w.Write([]byte(`{"key":"` + key + `"}`)) //nolint:errcheck // best effort write
	default:
...
func (s *Server) handleConnections(w http.ResponseWriter, r *http.Request) {
	...
	w.Header().Set("Content-Type", "application/json")
	_, _ = w.Write([]byte(`{"id":"` + id + `"}`)) //nolint:errcheck // best effort write
}
...
func (s *Server) handleAliases(w http.ResponseWriter, r *http.Request) {
	...
	case http.MethodGet:
		...
		w.Header().Set("Content-Type", "application/json")
		_ = json.NewEncoder(w).Encode(aliases) //nolint:errcheck // best effort encode
	case http.MethodPost:
		var req struct {
...
func (s *Server) handleChat(w http.ResponseWriter, r *http.Request) {
	...
	_ = handlers.ChatWithOptions(r.Context(), w, body, s.st, s.exec, s.fb, handlers.ChatOptions{ //nolint:errcheck // handler writes error response
		TokenSaver:      ts,
...
func (s *Server) handleImageGeneration(w http.ResponseWriter, r *http.Request) {
	...
	_ = handlers.ImageGeneration(r.Context(), w, body, s.st, s.exec, s.fb) //nolint:errcheck // handler writes error response
}
...
func (s *Server) handleTTS(w http.ResponseWriter, r *http.Request) {
	...
	_ = handlers.TTS(r.Context(), w, body, s.st, s.exec, s.fb) //nolint:errcheck // handler writes error response
}
...
func (s *Server) handleSTT(w http.ResponseWriter, r *http.Request) {
	...
	if strings.HasPrefix(ct, "multipart/") {
		_ = handlers.STTMultipart(ctx, w, r, s.st, s.fb) //nolint:errcheck // handler writes error response
		return
	}
	...
	_ = handlers.STT(ctx, w, body, s.st, s.exec, s.fb) //nolint:errcheck // handler writes error response
}
...
func (s *Server) handleVoices(w http.ResponseWriter, r *http.Request) {
	...
	_ = handlers.Voices(r.Context(), w, s.st) //nolint:errcheck // handler writes error response
}
...
func (s *Server) handleVideo(w http.ResponseWriter, r *http.Request) {
	...
	_ = handlers.Video(r.Context(), w, r, body, s.st, s.exec, s.fb) //nolint:errcheck // handler writes error response
}
...
func (s *Server) handleVideoPoll(w http.ResponseWriter, r *http.Request) {
	...
	_ = handlers.VideoPoll(r.Context(), w, r, r.PathValue("id"), s.st, s.fb) //nolint:errcheck // handler writes error response
}
...
func (s *Server) handleOAuth(w http.ResponseWriter, r *http.Request) {
	...
	s.dispatchOAuthAction(w, r, provider, action)
}
...
func (s *Server) dispatchOAuthAction(w http.ResponseWriter, r *http.Request, provider, action string) {
	switch action {
	case "authorize":
		s.handleOAuthAuthorize(w, r, provider)
	case "device-code":
		s.handleOAuthDeviceCode(w, r, provider)
	case "callback":
		s.oauth.HandleCallback(w, r, provider)
	case "start-proxy", "poll-status", "stop-proxy":
		s.dispatchOAuthProxy(w, r, provider, action)
	case "device", "poll", "exchange", "manual-code", "refresh":
		s.dispatchOAuthToken(w, r, provider, action)
	default:
		http.Error(w, `{"error":"unknown action"}`, http.StatusBadRequest)
	}
}

func (s *Server) dispatchOAuthProxy(w http.ResponseWriter, r *http.Request, provider, action string) {
	switch action {
	case "start-proxy":
		s.handleOAuthStartProxy(w, r, provider)
	case "poll-status":
		handleOAuthPollStatus(w, r, provider)
	case "stop-proxy":
		handleOAuthStopProxy(w, provider)
	}
}

func (s *Server) dispatchOAuthToken(w http.ResponseWriter, r *http.Request, provider, action string) {
	switch action {
	case "device", "poll":
		s.handleOAuthDevicePoll(w, r, provider)
	case "exchange":
		s.handleOAuthExchange(w, r, provider)
	case "manual-code":
		s.handleOAuthManualCode(w, r, provider)
	case "refresh":
		s.handleOAuthRefresh(w, r, provider)
	}
}

func (s *Server) handleOAuthAuthorize(w http.ResponseWriter, r *http.Request, provider string) {
...
func (s *Server) handleOAuthDeviceCode(w http.ResponseWriter, r *http.Request, provider string) {
...
func (s *Server) handleOAuthStartProxy(w http.ResponseWriter, r *http.Request, provider string) {
	...
	appPort, _ := strconv.Atoi(appPortStr) //nolint:errcheck
	...
	if ok, _ := result["success"].(bool); ok && state != "" && cv != "" && ru != "" { //nolint:errcheck // safe type assertion
...
func handleOAuthPollStatus(w http.ResponseWriter, r *http.Request, provider string) {
	if provider != "codex" && provider != "xai" {
	...
	st, _ := session["status"].(string) //nolint:errcheck // safe type assertion
	if st == "done" || st == "error" {
...
func handleOAuthStopProxy(w http.ResponseWriter, provider string) {
	if provider != "codex" && provider != "xai" {
		writeJSON(w, http.StatusBadRequest, map[string]any{"error": "Proxy only supported for codex/xai"})
		return
	}

	_ = oauth.StopOAuthProxy(provider) //nolint:errcheck

	writeJSONOK(w, map[string]any{"success": true})
}

type oauthDevicePollReq struct {
...
func (s *Server) handleOAuthDevicePoll(w http.ResponseWriter, r *http.Request, provider string) {
	if r.Method != http.MethodPost {
		http.Error(w, `{"error":"method not allowed"}`, http.StatusMethodNotAllowed)
		return
	}

	var req oauthDevicePollReq
	...
	if provider == "github" || provider == "copilot" {
		s.handleOAuthGithubPoll(w, r, provider, dc)
		return
	}
...
func (s *Server) handleOAuthExchange(w http.ResponseWriter, r *http.Request, provider string) {
	if r.Method != http.MethodPost {
		http.Error(w, `{"error":"method not allowed"}`, http.StatusMethodNotAllowed)
		return
	}
...
func (s *Server) handleOAuthManualCode(w http.ResponseWriter, r *http.Request, provider string) {
	if r.Method != http.MethodPost {
		http.Error(w, `{"error":"method not allowed"}`, http.StatusMethodNotAllowed)
		return
	}
	...
	if err != nil {
		...
		_ = oauth.StopOAuthProxy("xai") //nolint:errcheck
...
func (s *Server) handleOAuthRefresh(w http.ResponseWriter, r *http.Request, provider string) {
	...
	if provider == "kiro" {
		s.handleOAuthRefreshKiro(w, r, req.RefreshToken, req.PSD)
		return
	}

	s.handleOAuthRefreshGeneric(w, r, provider, req.RefreshToken)
}

func (s *Server) handleOAuthRefreshKiro(w http.ResponseWriter, r *http.Request, rt string, psd map[string]any) {
	tok, err := oauth.RefreshKiroToken(r.Context(), rt, psd)
	if err != nil {
		token, err2 := s.refresh.Refresh(r.Context(), "kiro", rt)
		if err2 != nil {
...
func (s *Server) handleOAuthRefreshGeneric(w http.ResponseWriter, r *http.Request, provider, rt string) {
	token, err := s.refresh.Refresh(r.Context(), provider, rt)
	if err != nil {
		tok, err2 := s.oauth.RefreshToken(r.Context(), provider, rt)
		if err2 != nil {
			http.Error(w, `{"error":"`+err.Error()+`"}`, http.StatusInternalServerError)
			return
		}
