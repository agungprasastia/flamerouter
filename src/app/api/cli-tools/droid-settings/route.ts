...
// Check if settings has FlameRouter customModels
...
// POST - Update FlameRouter customModels (merge with existing settings)
...
export async function POST(request) {
  try {
    ...
    // Remove all existing FlameRouter configs
    settings.customModels = settings.customModels.filter(
      (m) => !m.id?.startsWith("custom:FlameRouter"),
    );
...
// DELETE - Remove FlameRouter customModels only (keep other settings)
export async function DELETE() {
  try {
    ...
    // Remove FlameRouter customModels
    if (settings.customModels) {
      settings.customModels = settings.customModels.filter(
        (m) => !m.id?.startsWith("custom:FlameRouter"),
      );
