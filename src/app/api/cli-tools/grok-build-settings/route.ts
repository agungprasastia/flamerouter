...
const checkGrokInstalled = async () => {
  try {
    ...
    await execAsync(isWindows ? "where grok" : "which grok", {
      windowsHide: true,
    });
    return true;
  } catch {
...
const normalizeSubagentModels = (value) => {
  ...
  for (const type of GROK_SUBAGENT_TYPES) {
    ...
    const model =
      typeof entry === "string" ? entry.trim() : entry?.model?.trim();
...
const hasFlameRouterConfig = (settings) => Boolean(settings?.model?.base_url);
...
export async function GET() {
  try {
    ...
    return NextResponse.json(
      { error: "Failed to check grok-build settings" },
      { status: 500 },
    );
  }
}
...
export async function POST(request) {
  try {
    const { baseUrl, apiKey, model, contextWindow, subagentModels } =
      await request.json();
    ...
    return NextResponse.json({
      ...
      configPath: getGrokConfigPath(),
      modelSlot: "flamerouter",
    });
  } catch (error) {
    console.log("Error updating grok-build settings:", error);
    return NextResponse.json(
      { error: "Failed to update grok-build settings" },
      { status: 500 },
    );
  }
}
...
export async function DELETE() {
  try {
    ...
    return NextResponse.json({
      success: true,
      message: "flamerouter model slots removed from Grok Build",
    });
  } catch (error) {
    console.log("Error resetting grok-build settings:", error);
    return NextResponse.json(
      { error: "Failed to reset grok-build settings" },
      { status: 500 },
    );
  }
}
