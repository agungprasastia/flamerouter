...
const hasFlameRouterConfig = (config) => {
  ...
  return config.some((entry) => entry.name === "FlameRouter");
};

const getFlameRouterEntry = (config) => {
  if (!Array.isArray(config)) return null;
  return config.find((entry) => entry.name === "FlameRouter") || null;
};
...
export async function GET() {
  try {
    ...
    const entry = getFlameRouterEntry(config);
...
// POST - Apply FlameRouter config to chatLanguageModels.json
export async function POST(request) {
  try {
    ...
    // Replace existing FlameRouter entry or append
    const idx = config.findIndex((e) => e.name === "FlameRouter");
    if (idx >= 0) {
      config[idx] = newEntry;
    } else {
...
// DELETE - Remove FlameRouter entry from chatLanguageModels.json
export async function DELETE() {
  try {
    ...
    config = config.filter((e) => e.name !== "FlameRouter");
    await fs.writeFile(configPath, JSON.stringify(config, null, 2));

    return NextResponse.json({
      success: true,
      message: "FlameRouter removed from Copilot config",
    });
  } catch (error) {
