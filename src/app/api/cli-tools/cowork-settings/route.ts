...
const getCliToken = () => getConsistentMachineId(CLI_TOKEN_SALT);
...
export async function GET() {
  try {
    ...
    if (Array.isArray(config?.inferenceModels)) {
      for (const model of config.inferenceModels) {
    ...
    const managedMcp = Array.isArray(config?.managedMcpServers)
      ...
      : [];
    const hasFlameRouter = !!(
      config?.inferenceProvider === PROVIDER && baseUrl
    );

    // Active local plugins and custom bridge entries, collected in one pass.
    const stdioNames = new Set(LOCAL_STDIO_PLUGINS.map((p) => p.name));
    const activeLocalNames = [];
    const activeCustomPlugins = [];
    const coworkPlugins = [];
    for (const m of managedMcp) {
      const isLocalBridge =
        typeof m.url === "string" && m.url.includes("/api/mcp/");
      if (stdioNames.has(m.name) && isLocalBridge) {
        activeLocalNames.push(m.name);
        continue;
      }
      if (m.custom || (!stdioNames.has(m.name) && isLocalBridge)) {
        activeCustomPlugins.push({
          name: m.name,
          url: m.url,
