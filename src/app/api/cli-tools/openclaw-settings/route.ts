...
// Check if settings has FlameRouter config
const hasFlameRouterConfig = (settings) => {
  if (!settings || !settings.models || !settings.models.providers) return false;
  return !!settings.models.providers["flamerouter"];
};

// Read per-agent models.json and return current model id (without "flamerouter/" prefix)
const readAgentModel = async (agentDir) => {
  try {
    const modelsPath = path.join(agentDir, "models.json");
    const content = await fs.readFile(modelsPath, "utf-8");
    const data = JSON.parse(content);
    const models = data?.providers?.["flamerouter"]?.models;
...
export async function GET() {
  try {
    ...
    return NextResponse.json({
      ...
      hasFlameRouter: hasFlameRouterConfig(settings),
...
const writeAgentModels = async (agentDir, model, baseUrl, apiKey) => {
  ...
  existing.providers["flamerouter"] = {
    baseUrl,
...
// POST - Update FlameRouter settings (merge with existing settings)
export async function POST(request) {
  try {
    ...
    const normalizedBaseUrl = baseUrl.endsWith("/v1")
      ? baseUrl
      : `${baseUrl}/v1`;
    const fullModelId = `flamerouter/${model}`;

    // Remove all old flamerouter/* entries from agents.defaults.models
    Object.keys(settings.agents.defaults.models)
      .filter((k) => k.startsWith("flamerouter/"))
      .forEach((k) => {
    ...
    // Add fresh flamerouter models to allowlist
    allModelIds.forEach((m) => {
      settings.agents.defaults.models[`flamerouter/${m}`] = {};
    });

    // Remove old flamerouter model from each agent in agents.list. The
    ...
    if (settings.agents.list) {
      settings.agents.list = settings.agents.list.map((agent) => {
        if (resolveAgentModel(agent.model).startsWith("flamerouter/")) {
    ...
    // Update models.providers.flamerouter with all models
    settings.models.providers["flamerouter"] = {
    ...
    if (settings.agents.list) {
      settings.agents.list = settings.agents.list.map((agent) => {
        ...
        if (agentModel) return { ...agent, model: `flamerouter/${agentModel}` };
        return agent;
      });
...
// DELETE - Remove FlameRouter settings only (keep other settings)
export async function DELETE() {
  try {
    ...
    // Remove FlameRouter from models.providers
    if (settings.models && settings.models.providers) {
      delete settings.models.providers["flamerouter"];
    ...
    // Reset agents.defaults.model.primary if it uses flamerouter
    if (settings.agents?.defaults?.model?.primary?.startsWith("flamerouter/")) {
    ...
    return NextResponse.json({
      success: true,
      message: "FlameRouter settings removed successfully",
    });
  } catch (error) {
