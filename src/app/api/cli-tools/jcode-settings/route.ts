...
const getProviderEnvPath = () => {
  ...
  return path.join(configDir, "jcode", "provider-flamerouter.env");
};
...
const hasFlameRouterConfig = (config) => {
  ...
  if (providers["flamerouter"]) return true;
...
export async function GET() {
  ...
  const hasFlameRouter = hasFlameRouterConfig(config);
...
export async function POST(request) {
  try {
    ...
    return NextResponse.json({
      success: true,
      message:
        "jcode configured successfully. Use: jcode --provider-profile flamerouter",
      configPath: getConfigPath(),
    });
  } catch (error) {
...
export async function DELETE() {
  try {
    ...
    delete config.providers["flamerouter"];
    ...
    return NextResponse.json({
      success: true,
      message: "flamerouter configuration removed from jcode",
    });
  } catch (error) {
