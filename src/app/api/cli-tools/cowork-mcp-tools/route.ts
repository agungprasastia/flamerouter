...
function validateMcpUrl(value) {
  ...
  if (parsed.protocol !== "https:" && parsed.hostname !== "localhost" && parsed.hostname !== "127.0.0.1" && parsed.hostname !== "[::1]") return null;
  return parsed.href;
}
    ...
    const initRes = await fetch(url, {
      ...
      body: JSON.stringify({
        ...
        params: {
          ...
          capabilities: {},
          clientInfo: { name: "flamerouter", version: "1" },
        },
      }),
      signal: ac.signal,
      redirect: "error",
...
export async function POST(request) {
  try {
    ...
    const targetUrl = validateMcpUrl(url);
    if (!targetUrl) {
      return NextResponse.json(
        { error: "url must use HTTPS or loopback HTTP" },
        { status: 400 },
      );
    }
    const result = await probeMcp(targetUrl);
