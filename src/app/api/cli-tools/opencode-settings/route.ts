...
const hasFlameRouterConfig = (config) => {
  if (!config?.provider) return false;
  return !!config.provider["flamerouter"];
};
...
export async function GET() {
  try {
    ...
    const providerConfig = config?.provider?.["flamerouter"];
    const modelMap = providerConfig?.models || {};

    return NextResponse.json({
      installed: true,
      config,
      hasFlameRouter: hasFlameRouterConfig(config),
      configPath: getConfigPath(),
      opencode: {
        models: Object.keys(modelMap),
        activeModel: config?.model?.startsWith("flamerouter/")
          ? config.model.replace(/^flamerouter\//, "")
          : null,
...
// POST - Apply FlameRouter as openai-compatible provider (multi-model support)
export async function POST(request) {
  try {
    ...
    const normalizedBaseUrl = baseUrl.endsWith("/v1")
      ? baseUrl
      : `${baseUrl}/v1`;
    const keyToUse = apiKey || "sk_flamerouter";
    const effectiveSubagentModel = subagentModel || modelsArray[0];

    // Ensure provider object
    if (!config.provider) config.provider = {};

    // Preserve any existing flamerouter provider entry and its models
    const existingProvider = config.provider["flamerouter"] || {
    ...
    config.provider["flamerouter"] = existingProvider;
    ...
    if (activeModel === "") {
      ...
      if (finalActive) {
        config.model = `flamerouter/${finalActive}`;
      }
    }
    ...
    config.agent.explorer = {
      ...
      mode: "subagent",
      model: `flamerouter/${effectiveSubagentModel}`,
    };
...
export async function PATCH(request) {
  try {
    ...
    if (clearActiveModel === true) {
      ...
      if (config.model?.startsWith("flamerouter/")) {
        config.model = "";
      }
    }
...
// DELETE - Remove FlameRouter provider or specific models from config
export async function DELETE(request) {
  try {
    ...
    if (modelToRemove && config.provider?.["flamerouter"]?.models) {
      delete config.provider["flamerouter"].models[modelToRemove];

      // If no models left, remove the provider
      if (Object.keys(config.provider["flamerouter"].models).length === 0) {
        delete config.provider["flamerouter"];
        if (config.model?.startsWith("flamerouter/")) delete config.model;
      } else if (config.model === `flamerouter/${modelToRemove}`) {
        // If removed model was active, switch to first remaining model
        const remainingModels = Object.keys(
          config.provider["flamerouter"].models,
        );
        config.model = `flamerouter/${remainingModels[0]}`;
      }
    } else {
      // No specific model - remove entire flamerouter provider
      if (config.provider) delete config.provider["flamerouter"];
      if (config.model?.startsWith("flamerouter/")) delete config.model;
    }

    // Remove subagent configuration
    if (config.agent?.explorer?.model?.startsWith("flamerouter/")) {
    ...
    return NextResponse.json({
      ...
      message: modelToRemove
        ...
        : "FlameRouter settings removed from OpenCode",
    });
  } catch (error) {
