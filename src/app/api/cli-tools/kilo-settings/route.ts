...
const hasFlameRouterConfig = (auth) => {
  if (!auth) return false;
  const entry = auth["openai-compatible"] || auth["flamerouter"];
  if (!entry) return false;
  const baseUrl = entry.baseUrl || entry.baseURL || "";
  return (
    baseUrl.includes("localhost") ||
    baseUrl.includes("127.0.0.1") ||
    baseUrl.includes("flamerouter")
  );
};
...
export async function GET() {
  try {
    ...
    return NextResponse.json({
      ...
      hasFlameRouter: hasFlameRouterConfig(auth),
...
export async function DELETE() {
  try {
    ...
    return NextResponse.json({
      success: true,
      message: "FlameRouter settings removed from Kilo Code",
    });
  } catch (error) {
