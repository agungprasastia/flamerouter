...
const PROVIDER_NAME = "flamerouter";
...
// Build TOML config for FlameRouter (openai provider mode)
const buildFlameRouterConfig = (baseUrl, apiKey, model) => {
...
// Detect FlameRouter by checking if provider is "openai" and base_url points to localhost/127.0.0.1
...
export async function POST(request) {
  try {
    ...
    const newConfig = buildFlameRouterConfig(
      baseUrl,
      apiKey || "sk_flamerouter",
      model,
    );
