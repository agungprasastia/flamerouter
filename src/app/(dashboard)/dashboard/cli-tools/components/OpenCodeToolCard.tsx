...
export default function OpenCodeToolCard({
  ...
  useEffect(() => {
    ...
    if (status?.config?.agent?.explorer?.model?.startsWith("flamerouter/")) {
      setSubagentModel(
        status.config.agent.explorer.model.replace("flamerouter/", ""),
      );
    }
  }, [status]);
  ...
  const getConfigStatus = () => {
    ...
    if (!status.hasFlameRouter) return "not_configured";
    const url =
      status.config?.provider?.["flamerouter"]?.options?.baseURL || "";
        ...
        content: JSON.stringify(
          {
            ...
            model: `flamerouter/${activeModelToShow}`,
            agent: {
              explorer: {
          ...
          {!checking && status?.installed && (
            <>
              <div className="flex flex-col gap-2">
                ...
                {/* Current configured */}
                {status?.config?.provider?.["flamerouter"]?.options
                  ?.baseURL && (
