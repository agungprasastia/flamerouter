...
const SUBAGENT_TYPES = [
  ...
  },
  {
    id: "explore",
    label: "Explore",
    help: "Read-only codebase research and investigation",
  },
  {
    id: "plan",
    label: "Plan",
    help: "Architecture and implementation planning",
  },
];
  ...
  return (
    <div className="grid grid-cols-1 gap-1.5 sm:grid-cols-[8rem_auto_1fr_auto] sm:items-center sm:gap-2">
      <div className="sm:text-right">
        <span className="text-xs font-semibold text-text-main sm:text-sm">
          {label}
        </span>
        {help && (
...
export default function GrokBuildToolCard({
  ...
  const hydrateForm = useCallback((status) => {
    ...
    const configuredSubagents = Object.fromEntries(
      SUBAGENT_TYPES.map((type) => [
        type.id,
        status?.settings?.subagentModels?.[type.id]?.model,
      ]).filter(([, model]) => Boolean(model)),
    );
  ...
  useEffect(() => {
    ...
    const synchronize = async () => {
      if (!statusHydrated) await checkStatus({ hydrate: true });
  ...
  }, [isExpanded, checkStatus, fetchModelAliases, statusHydrated]);
  ...
  const handleModelSelect = (model) => {
    if (modelTarget === "main") {
      ...
      setSubagentModels((current) => ({
        ...current,
        [modelTarget]: model.value,
      }));
    }
    setModelTarget(null);
  };
  ...
  const getManualConfigs = () => {
    const keyToUse =
      selectedApiKey?.trim() ||
      (!cloudEnabled ? "sk_flamerouter" : "<API_KEY_FROM_DASHBOARD>");
    ...
    for (const type of SUBAGENT_TYPES) {
      ...
      blocks.push(
        `[model.${slot}]\nmodel = "${model}"\nbase_url = "${baseUrl}"\nname = "FlameRouter ${type.id}"\ndescription = "Routed via FlameRouter gateway"\napi_backend = "chat_completions"\napi_key = "${keyToUse}"\ncontext_window = ${getContextWindow(model) || 200000}`,
      );
    }
    if (mappings.length)
      blocks.splice(1, 0, `[subagents.models]\n${mappings.join("\n")}`);
    return [
      { filename: "~/.grok/config.toml", content: `${blocks.join("\n\n")}\n` },
    ];
  };
            ...
            <>
                ...
                {tool.notes?.length > 0 && (
                  <div className="mb-2 flex flex-col gap-2">
                    {tool.notes.map((note, index) => (
                      <div
                        key={index}
                        className={`flex items-start gap-2 rounded p-2 text-xs ${note.type === "warning" ? "bg-yellow-500/10 text-yellow-600 dark:text-yellow-400" : "bg-blue-500/10 text-blue-600 dark:text-blue-400"}`}
                      >
                  ...
                  <div className="grid grid-cols-1 gap-1.5 sm:grid-cols-[8rem_auto_1fr] sm:items-center sm:gap-2">
                    ...
                    <span className="min-w-0 truncate rounded bg-surface/40 px-2 py-2 text-xs text-text-muted sm:py-1.5">
                      ...
                      {configuredModel.context_window
                        ? ` · ${(configuredModel.context_window / 1000).toLocaleString()}K ctx`
                        : ""}
                    </span>
                  </div>
                )}
                  ...
                  <div className="mb-2 flex items-start gap-2">
                    ...
                    <div>
                      ...
                      <p className="text-[10px] text-text-muted">
                        Leave blank to inherit Main Model. Each override keeps
              ...
              <div className="flex flex-col sm:flex-row sm:items-center gap-2">
                ...
                <Button
                  ...
                  size="sm"
                  onClick={handleReset}
                  disabled={!grokStatus?.hasFlameRouter}
                  loading={restoring}
                  className="w-full sm:w-auto"
                >
                  <span className="material-symbols-outlined text-[14px] mr-1">
        ...
        <ModelSelectModal
          ...
          title={
            modelTarget === "main"
              ...
              : `Select ${SUBAGENT_TYPES.find((type) => type.id === modelTarget)?.label || "Subagent"} Model`
          }
        />
      )}
