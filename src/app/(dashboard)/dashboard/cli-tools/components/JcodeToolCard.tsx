...
export interface JcodeStatus {
  installed?: boolean;
  hasFlameRouter?: boolean;
  config?: {
    providers?: Record<string, { base_url?: string; default_model?: string; [key: string]: unknown }>;
  };
  envApiKey?: string;
  error?: string;
  [key: string]: unknown;
...
export interface JcodeToolCardProps {
  tool: ToolCardDef;
  isExpanded?: boolean;
  onToggle?: () => void;
  baseUrl?: string;
...
export default function JcodeToolCard({
  ...
  const getConfigStatus = () => {
    ...
    if (!jcodeStatus?.hasFlameRouter) return "not_configured";
    const currentProvider = jcodeStatus.config?.providers?.["flamerouter"];
    if (!currentProvider || !currentProvider.base_url) return "not_configured";
  ...
  useEffect(() => {
    if (jcodeStatus?.installed && !hasInitializedModel.current) {
      ...
      const provider = jcodeStatus.config?.providers?.["flamerouter"];
      if (provider) {
  ...
  const checkJcodeStatus = async () => {
    ...
    try {
      ...
      const data: JcodeStatus = await res.json();
      setJcodeStatus(data);
    } catch (error: unknown) {
      const err = error as { message?: string };
      setJcodeStatus({ installed: false, error: err.message });
    } finally {
  ...
  const getManualConfigs = () => {
    ...
    const configToml = `[providers.flamerouter]
type = "openai-compatible"
base_url = "${getEffectiveBaseUrl()}"
auth = "bearer"
api_key_env = "JCODE_9ROUTER_API_KEY"
env_file = "provider-flamerouter.env"
default_model = "${selectedModel || "cc/claude-opus-4-7"}"
requires_api_key = true

[[providers.flamerouter.models]]
id = "${selectedModel || "cc/claude-opus-4-7"}"`;
    ...
    return [
      ...
      {
        filename: "~/.config/jcode/provider-flamerouter.env",
        content: envContent,
      },
    ];
  };
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div className="flex-1">
                    ...
                    <p className="text-sm text-text-muted mt-2">
                      Manual configuration is still available if flamerouter is
          ...
          {!checkingJcode && jcodeStatus?.installed && (
            <>
              <div className="flex flex-col gap-2">
                ...
                {/* Current configured */}
                {jcodeStatus?.config?.providers?.["flamerouter"]?.base_url && (
                ...
                <div className="flex flex-col gap-1 p-3 bg-blue-500/5 border border-blue-500/20 rounded-lg">
                  ...
                  <code className="text-xs font-mono text-text-muted">
                    jcode --provider-profile flamerouter
                  </code>
                  <code className="text-xs font-mono text-text-muted">
                    jcode --provider-profile flamerouter --model{" "}
              ...
              <div className="grid grid-cols-1 gap-2 sm:flex sm:items-center">
                ...
                <Button
                  ...
                  disabled={!jcodeStatus?.hasFlameRouter}
                  loading={restoring}
                >
                  <span className="material-symbols-outlined text-[14px] mr-1">
                    restore
