...
export default function DroidToolCard({
  ...
  const getConfigStatus = () => {
    ...
    // Check for any FlameRouter model entry (support multi-model: custom:FlameRouter-0, custom:FlameRouter-1, ...)
  ...
  useEffect(() => {
    if (droidStatus?.installed && !hasInitializedModel.current) {
      ...
      if (existingModels.length > 0) {
        ...
        // Legacy: single model stored as custom:FlameRouter-0
        const legacy = droidStatus.settings?.customModels?.find(
          (m) => m.id === "custom:FlameRouter-0",
        );
        if (legacy?.model) {
            ...
            <>
              ...
              <div className="grid grid-cols-1 gap-2 sm:flex sm:items-center">
                ...
                <Button
                  ...
                  disabled={!droidStatus?.hasFlameRouter}
                  loading={restoring}
                >
                  <span className="material-symbols-outlined text-[14px] mr-1">
                    restore
