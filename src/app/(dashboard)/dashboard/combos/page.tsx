...
export default function CombosPage() {
  ...
  return (
    <div className="flex min-w-0 flex-col gap-8 px-1 sm:px-0 workbench-reveal">
      <div className="workbench-intro">
        <div>
          <p className="workbench-kicker">Route policy / Fallback chains</p>
          <h2 className="mt-3 text-3xl font-semibold tracking-[-0.04em]">
            Compose resilient routes.
          </h2>
        </div>
        <p className="text-sm leading-relaxed text-text-muted">
          Order models by intent, then let FlameRouter move through fallback
          candidates.
        </p>
      </div>
      {/* Header */}
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div className="min-w-0">
          ...
          <p className="text-xs text-text-muted mt-0.5">
            Your model can&apos;t read image/audio? Auto-switches to a model in
            the pool below.
          </p>
          <ul className="mt-1.5 text-[11px] text-text-muted flex flex-col gap-0.5">
            <li>
              <span className="font-medium text-text-main">Vision</span> —
              images (png, jpg, webp, …)
            </li>
            <li>
              <span className="font-medium text-text-main">Audio</span> — audio
              input
            </li>
          </ul>
        </div>
      ...
      <div className="flex flex-col gap-4">
        {CAPACITY_ADAPTER_CAPS.map((cap) => (
          <CapacityAdapterCap
            ...
            onChange={(entry) =>
              onChange({ ...capacityAdapter, [cap.key]: entry })
            }
            activeProviders={activeProviders}
            getCaps={getCaps}
          />
