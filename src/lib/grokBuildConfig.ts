...
function deleteSectionField(
  ...
  if (!nextBody.trim())
    return toml.replace(match[0], "").replace(/\n{3,}/g, "\n\n");
  return toml.replace(match[0], `[${section}]\n${nextBody}`);
}
...
function parseModelSection(
  ...
  const body = match[1] || "";
  const contextWindow = getSectionNumber(
    toml,
    `model.${slot}`,
    "context_window",
  );
...
function buildModelSection({
  ...
  const lines = [
    ...
    `description = ${tomlString("Routed via FlameRouter gateway")}`,
...
function rememberPreviousDefault(toml: string): string {
  ...
  return insertMarker(
    toml,
    `# flamerouter-prev-default = ${tomlString(current)}\n`,
  );
}
...
function restorePreviousDefault(toml: string): string {
  ...
  if (
    getSectionField(next, MODELS_SECTION, "default") === GROK_MAIN_MODEL_SLOT
  ) {
...
function rememberPreviousSubagent(toml: string, type: string): string {
  ...
  return insertMarker(
    toml,
    `# flamerouter-prev-subagent-${type} = ${tomlString(previous)}\n`,
  );
}
...
function restorePreviousSubagent(toml: string, type: string): string {
  ...
  if (
    getSectionField(next, SUBAGENT_MODELS_SECTION, type) !== modelSlot(type)
  ) {
    return next;
  }
...
export function parseGrokBuildConfig(toml: string): ParsedGrokBuildConfig {
  ...
  for (const type of GROK_SUBAGENT_TYPES) {
    ...
    subagentModels[type] =
      mapping === modelSlot(type) ? parseModelSection(toml, mapping) : null;
  }
