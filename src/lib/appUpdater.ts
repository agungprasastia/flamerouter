  ...
  try {
    const mitmPidFile = path.join(
      process.platform === "win32"
        ? path.join(process.env.APPDATA || "", "flamerouter")
  ...
  if (platform === "win32") {
    try {
      ...
      lines.forEach((line) => {
        ...
        // Match anything running from flamerouter install dir or wrapper cli.js
        const isAppProcess =
          lower.includes("flamerouter") ||
...
function resolveBundledUpdaterPath(): string {
  ...
  // Dev: cwd is app/
  const fromCwd = path.join(process.cwd(), "src", "lib", "updater", "updater");
...
function ensureRuntimeUpdater(bundledPath: string): string {
  try {
    ...
    const runtimePath = path.join(runtimeDir, "updater");
