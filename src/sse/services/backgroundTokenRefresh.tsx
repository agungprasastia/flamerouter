...
import { getRefreshLeadMs } from "open-sse/services/tokenRefresh";
import { getCredentialExpiryMs } from "open-sse/services/oauthCredentialManager";
...
async function loadActiveConnections() {
  ...
  const { getProviderConnections } =
    await import("../../lib/db/repos/connectionsRepo");
    ...
    await Promise.allSettled(
      due.map(async (conn) => {
        try {
          ...
          log.warn(
            "BG_TOKEN_REFRESH",
            "Connection refresh failed (swallowed)",
            {
              id: conn?.id,
              provider: conn?.provider,
...
export function startBackgroundTokenRefresh({ intervalMs } = {}) {
  ...
  if (isTruthyEnv(process.env.DISABLE_BACKGROUND_TOKEN_REFRESH)) {
    log.info(
      "BG_TOKEN_REFRESH",
      "Disabled via DISABLE_BACKGROUND_TOKEN_REFRESH",
    );
    return false;
  }
  if (isNonServerRuntime()) {
    log.debug(
      "BG_TOKEN_REFRESH",
      "Skip start outside long-running server runtime",
    );
    return false;
  }
