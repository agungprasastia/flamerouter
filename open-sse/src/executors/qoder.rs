//! Qoder executor — COSY signed inference endpoint.
...
pub async fn execute(
    ...
    let token = credentials
        ...
        .ok_or_else(|| anyhow!("missing Qoder token"))?;

    let base = crate::providers::base_url_for(provider);
    let url = if base.is_empty() {
        "https://api3.qoder.sh/algo/api/v2/service/pro/sse/agent_chat_generation".to_string()
    } else {
        base
    };
